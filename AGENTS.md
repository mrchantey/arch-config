...
`assets/` is a manifest of symlinks, one per noun, each pointing at where that noun lives on the machine: today only `assets/pictures -> ~/Pictures/assets` (wallpapers, headshots, the omarchy-logo template). The tree is mirrored to the `arch-config--shared--assets` S3 bucket under the same prefixes (`pictures/...`), shared across machines and tracked in no repo. The bucket is declared in `main.bsx` and driven by beet: `just assets validate|plan|deploy|destroy` provision it, and `just push-assets` (`just assets push`) mirrors the tree up with delete, so a file removed locally is removed from the bucket on the next push. beet refuses to push an empty or unhydrated link, which is what makes the delete safe, and object versioning keeps overwritten and deleted objects for 90 days. The `assets` recipe runs the stock `beet` binary out of `~/me/beet` with the `infra,extra` features, so the first push on a machine builds it; the tofu work dir lands in `target/` (gitignored) and the state in the shared `beet-state` bucket, so the verbs need AWS credentials and `tofu`.

Hydrating does not go through beet. `just pull-assets` is a plain `aws s3 sync --no-sign-request`, because `init-user` runs it on a fresh machine before any Rust toolchain or AWS credentials exist; the bucket's public read grants `ListBucket` as well as `GetObject`, which is what lets an unsigned sync list before it gets. Individual files are fetchable unsigned at `https://arch-config--shared--assets.s3.us-west-2.amazonaws.com/pictures/<path>`, which is how `stow-files-init` gets the Firewatch wallpaper. Adding a noun is one more symlink under `assets/` pointing at the directory that owns it, plus a `mkdir -p` of that target in `pull-assets`, since the aws cli cannot write through a dangling link.

This replaced the SST-managed `mrchantey-os` bucket on 2026-09-22, the last SST resource in the account; `~/Pictures/shared` became `~/Pictures/assets` at the same time, so a machine set up before then needs the `mv` before its first pull.
